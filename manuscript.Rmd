...
When a confidence interval is used, the SGPV and equivalence tests such as the Two One-Sided Tests (TOST) procedure [@lakens_equivalence_2017;@meyners_equivalence_2012;@schuirmann_comparison_1987] appear to have close ties, because both tests conpare a confidence interval against an equivalence range.
...
As the observed mean in a one-sample *t*-test lies closer to the test value (in the plot below, from situation A to D, the mean gets closer to the test value by 0.1) the difference in the overlap changes uniformly.
```{r, include=FALSE}
m <- 146
mu <- 144.5
sd <- 500
...
The *p*-value from the TOST procedure and the SGPV are also unrelated when the CI is wider than the equivalence range (so the precision is low) and overlaps with the upper and lower equivalence bound, but the CI is *not* twice as wide as the equivalence range. For example, in the example below, we see that the CI is only 1.79 times as wide as the equivalence bounds, but the CI overlaps with the lower and upper equivalence bounds. This means the SGPV is not set to 0.5, but it is constant across a range of observed means.
```{r, include=FALSE}
m <- 0.2
n <- 10
sd <- 1
...
To conclude this section, there are four situations where the *p*-value from the TOST procedure is unrelated to the SGPV. In all these situations the *p*-value for the equivalence test differentiates tests with different means, but the SGPV does not. Therefore, as a purely continuous descriptive statistic, the SGPV is more limited than the *p*-value from the TOST procedure. 

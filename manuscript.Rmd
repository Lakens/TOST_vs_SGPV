...
abstract: |
  One way to move beyond the limitations of null-hypothesis tests statistical approaches have been developed where the observed data is compared against an range of values that are equivalent to the absence of a meaningful effect. We compare the behavior of the recently proposed second generation *p*-value [@blume_second-generation_2018] with equivalence testing using the Two-One-Sided Tests procedure [@schuirmann_comparison_1987]. We show the two approaches yield almost identical results under optimal circumstances, but under suboptimal circumstances (e.g., when the confidence interval is wider than the equivalence range, or with asymmetric confidence intervals) the second generation *p*-value becomes difficult to interpret as a descriptive statistic. The second generation is always interpretable in a dichotomous manner when it is 0 or 1, but this can easily be confirmed visually, and does not require calculation. Equivalence tests yield more consistent *p*-values and allow for a easy control of Type I and Type II error rates. 

keywords          : "equivalence testing, second generation *p*-values, *p*-values, hypothesis testing, statistics"
wordcount         : ""
...
When testing for equivalence researchers aim to examine whether the effect is practically zero, or that there is no meaningful effect. 
...
Second generation p-values (SGPV) were recently proposed to as a descriptive statistic that can loosely be interpreted as ‘the proportion of data-supported hypotheses that are also null hypotheses’ [@blume_second-generation_2018]. 
...
When a confidence interval is used, the SGPV and equivalence tests such as the Two One-Sided Tests (TOST) procedure [@lakens_equivalence_2017;@meyners_equivalence_2012;@schuirmann_comparison_1987] appear to have close ties. 
...
Two other points always have to overlap. When the 95% CI falls completely, but only just inside the equivalence region, the TOST (which relies on a one-sided test) should be significant at an alpha level of 0.025. When the SGPV changes from <1 to 1 the 95% CI is exactly equal to one of the equivalence bounds (see situation B in \ref{fig:TOSTSGPV3}, where the 95% CI falls completely inside the equivalence bounds) the TOST *p*-value is 0.025. The third point where the SGPV and the *p*-value from the TOST procedure should overlap is where the SGPV  changes from a positive value to 0 (i.e., when the 95% CI falls completely outside of the equivalence bound, see situation C in \ref{fig:TOSTSGPV3}). When the 95% CI touches the outside of the equivalence bound and the TOST *p*-value will be 0.975. 
...
```
```{r, TOSTSGPV4, fig.cap="Means, normal distribution, and 95% CI for samples where the true population mean is 1.5, 1.4, 1.3, and 1.2."}
plot(NA, 
     ylim = c(0, 1), 
     xlim = c(-3, 3),
     yaxt = "n",
...
If the observed mean would be somewhat closer to 0, or further away from 0, the SGPV would remain constant (the CI width does not change, it completely overlaps with the equivalence range) while the *p*-value for the TOST procedure can vary between 0 and .025. We can see this in \ref{fig:TOSTSGPV9} below. The SGPV is not set to 0.5, but is slightly higher than 0.5 across a range of means. How high the SGPV will be when the CI overlaps with the lower and upper equivalence bounds, but the CI is not twice as large as the equivalence range, depends on the width of the CI and the equivalence range. 
```{r, include=FALSE}
step = 0.01
...
The reason that the equivalence test and SGPV no longer overlap is also because of asymmetric confidence intervals. If the observed correlation falls exactly on the equivalence bound the *p*-value for the equivalence test indicates that the probability of observing the observed or more extreme data, assuming the equivalence bound is the true effect size, is 50%. In other words, if the true effect size is the same as the equivalence bound, it is equally likely to find an effect more extreme than the equivalence bound, as it is to observe an effect that is less extreme than the equivalence bound. However, as can be seen in \ref{fig:TOSTSGPV12}, the two second generation *p*-values associated with the observed correlations at r = -0.45 and r = 0.45 are `r round(sgpv,3)`. Because the confidence intervals are asymmetric around the observed effect size of 0.45 (ranging from `r round(res$LL_CI_TTEST,2)` to `r round(res$UL_CI_TTEST,2)`) according to @blume_second-generation_2018 `r round(sgpv,3)*100`% of the data-supported hypotheses are null hypotheses, and therefore `r round(sgpv,3)*100`% of the data-supported hypotheses are compatible with the null premise. 
